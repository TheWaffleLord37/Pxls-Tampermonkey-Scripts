...
(function() {
    ...
    async function handleShiftRightClick(event) {
        if (event.shiftKey && (event.which === 3 || event.button === 2)) { // Shift + Right-click event
    ...
    let waitForLoad = setInterval(function () {
        if (App) {
            ...
            pxls.board.addEventListener('contextmenu', handleShiftRightClick)
            ...
            window.addEventListener('keyup', function(event) {
                if (event.key === 'Shift') {
                    pxls.board.removeEventListener('contextmenu', handleShiftRightClick)
                }
            });
        }
    }, 1000)
